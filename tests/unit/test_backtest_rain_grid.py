"""Unit tests for the backtest rainfall-node grid helpers (pure)."""

from __future__ import annotations

from limen.cli.backtest import _build_rain_nodes, _nearest_node


def test_build_rain_nodes_covers_bbox() -> None:
    bbox = (16.0, 40.0, 16.5, 40.5)
    nodes = _build_rain_nodes(bbox, spacing=0.25)
...
def test_build_rain_nodes_degenerate_bbox_falls_back_to_centroid() -> None:
    ...
    nodes = _build_rain_nodes((16.0, 40.0, 16.0, 40.0), spacing=0.25)
    assert nodes == [(16.0, 40.0)]


def test_nearest_node_picks_closest() -> None:
    nodes = [(16.0, 40.0), (16.5, 40.0), (16.0, 40.5)]
    assert _nearest_node(16.05, 40.02, nodes) == 0
    assert _nearest_node(16.48, 40.03, nodes) == 1
    assert _nearest_node(16.02, 40.47, nodes) == 2
