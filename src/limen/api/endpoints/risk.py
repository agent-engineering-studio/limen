...
async def legend(response: Response, hazard: HazardType = DEFAULT_HAZARD) -> dict[str, Any]:
    ...
    return {
        ...
        # Built by the thresholds object, not here: the blocks differ per
        # hazard, and an endpoint that reached into them would need a branch
        # per danger and would break on the first one without a Caine block.
        "model": t.model_card(),
    }
