...
| The fluvial probe samples the basin, not the centroid | An AOI centroid almost never sits on a river GloFAS models — measured: 0.0 for Basilicata. `basin_max=True` probes the snapped lattice and takes the worst ratio among points whose baseline is ≥10 % of the AOI's largest, because a plain maximum picks the most volatile ditch (measured 62.0 where the real rivers were at 1.25). **Off for landslide on purpose**: the V1 champion was scored with the centroid signal, and changing it under the same name would move its numbers without a backtest. |
