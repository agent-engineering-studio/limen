...
| Wildfire = FWI × fuel × slope | The Canadian Fire Weather Index chain (Van Wagner 1987) in `core/scoring/wildfire/fwi.py` is pure and verified against the published reference day to 1e-9. A **weighted sum**, never a product: a bare-rock cell in extreme fire weather still scores, because the cell next to it will burn. |
| The FWI chain is per weather node | The three moisture codes depend only on the weather, which Open-Meteo serves at ~9 km, so `fwi_state` is keyed by `(node_lon, node_lat, day)` — NUMERIC, because float equality in a PK opens a second chain on rounding noise. Keying it by cell would store the same six numbers ~500 times a day per region. The grid step is `fwi.node_spacing_deg` in `wildfire.yaml`, **not** an env knob: changing it starts fresh chains and restarts the spin-up. Retention must never delete past days — the recursion walks through them. |
