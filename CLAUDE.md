...
| Flood signals are per node, never one per AOI | Both triggers *are* the flood engine, so a single number copied onto every cell is the failure this repo already documented for rainfall (13 mm at the Puglia centroid against 77 mm at the cells that failed). `per_node=True` samples the snapped lattice at 0.1° and each cell reads its nearest node; the AOI-level scalars stay for the landslide H component, which has always used them — changing those under the same names would move the V1 champion without a backtest. A node counts as carrying a river only if its trailing baseline is ≥10 % of the AOI's largest, because the ratio is pathological on trickles (measured 62.0 on a ditch where the real rivers sat at 1.25); the rest report `None`, which is "no watercourse here", not "a river running low". Requests go in batches of 100 — 677 points in one query string came back as something that was not JSON. |
